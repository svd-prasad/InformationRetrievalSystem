...
{  while(TRUE)
   {  int ch = getc(f);
      ...
      {  int i = 0;
        ...
        // printf("%s",s);                      s contains stemmed input
     ...
     //FILE * f = fopen(file,"r");
      //if (f == 0) { fprintf(stderr,"File %s not found\n",argv[i]); exit(1); }
              ...
              while(fscanf(f,"%s",str)!=EOF)
              {
                    ...
                    fclose(fp);
                    if(flag1==1) fprintf(fp1,"%s\n",str);
              } 
         fclose(f);
         fclose(fp1);
                
      fp1=fopen("steminput","r");
      stemfile(fp1);
      fclose(fp1);
...
/* 
 stem input contains afterstop words elimination
 stem contains output after porter stemming algorithm
*/    
